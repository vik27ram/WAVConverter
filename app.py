...
def upload():
	if request.method == 'POST':
		...
		if file and allowed_file(file.filename):
			...
			# Convert
			dfile = '{}.{}'.format(os.path.splitext(filename)[0], 'mid') # Build file name
			...
			with open(outputF, "wb") as output_file:
				MyMIDI.writeFile(output_file)
			# Send as a downloadable file
			ddir = os.path.join(current_app.root_path, app.config['DOWNLOAD_FOLDER'])
			return send_from_directory(directory=ddir, path=dfile, as_attachment=True)
